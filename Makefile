...
# Docker image generated by workflows and available for testing
LOCAL_TESTER_IMAGE ?= $(REPO)-test-runner:latest
...
test-deb:
	@echo "Running test for Debian build of $(REPO) with image $(LOCAL_TESTER_IMAGE)..."
	mkdir -p ${LOCAL_TESTER_RESULTS}
	echo "Test run id ${TEST_DATE}" > ${LOCAL_TESTER_RESULTS}/${TEST_ID}.log
...
# Validate included python code via a git pre-commit hook which runs `ruff`.
pre-commit: \
    .workflows-venv \
...
.workflows-venv:
	python3 -m venv $@
	$@/bin/pip install pre-commit ruff
